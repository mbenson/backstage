...
describe('Entity page', () => {
  ...
  describe('Entity Page Headers', () => {
    ...
    it('Should use a custom default header', async () => {
      const defaultEntityHeader = EntityHeaderBlueprint.make({
        name: 'default',
        params: {
          title: [<button>Copy to clipboard</button>],
          subtitle: 'wayback-archive',
        },
      });

      const tester = createExtensionTester(
        Object.assign({ namespace: 'catalog' }, catalogEntityPage),
      ).add(defaultEntityHeader);
      ...
      await waitFor(() =>
        expect(screen.queryByText(/artist-lookup/)).not.toBeInTheDocument(),
      );

      await waitFor(() =>
        expect(screen.getByText(/wayback-archive/)).toBeInTheDocument(),
      );
    ...
    it('Should use a custom header for entities of type api', async () => {
      const apiEntityHeader = EntityHeaderBlueprint.make({
        name: 'default',
        params: {
          defaultFilter: 'kind:api',
          title: { actions: [<button>Open playground</button>] },
        },
      });
      ...
      await waitFor(() =>
        expect(
          screen.queryByRole('button', { name: /Open playground/ }),
        ).not.toBeInTheDocument(),
      );
        ...
        <TestApiProvider
          apis={[
            [
              catalogApiRef,
              catalogApiMock.mock({
                getEntityByRef: async () => ({ ...entityMock, kind: 'API' }),
              }),
            ],
            [starredEntitiesApiRef, mockStarredEntitiesApi],
          ]}
